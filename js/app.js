  ...
  api('listUsers').then(res => {
    ...
    usersList.innerHTML = managed.length ? managed.map(u => {
      ...
      if(u.status === 'approved'){
        actions.push(`<button class="btn btn-ghost access-action" data-tooltip="Forçar troca de senha" onclick="forceUserPasswordChange('${u.id}','${encodedEmail}')" aria-label="Forçar troca de senha"><svg class="access-icon access-password-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><text x="12" y="16" text-anchor="middle" font-family="Arial, sans-serif" font-size="8.5" font-weight="700" letter-spacing="1.2" fill="currentColor">***</text></svg></button>`);
      }
      if(!isSelf){
